module Gesmew
  module Api
    module V1
      class StockLocationsController < Gesmew::Api::BaseController
        def index
          authorize! :read, StockLocation
          @stock_locations = StockLocation.accessible_by(current_ability, :read).inspection('name ASC').ransack(params[:q]).result.page(params[:page]).per(params[:per_page])
