module Gesmew
  module Api
    module V1
      class OptionValuesController < Gesmew::Api::BaseController
        def index
          if params[:ids]
        ...
        def create
          authorize! :create, Gesmew::OptionValue
        ...
        private
          ...
          def scope
            if params[:option_type_id]
              @scope ||= Gesmew::OptionType.find(params[:option_type_id]).option_values.accessible_by(current_ability, :read)
            else
              @scope ||= Gesmew::OptionValue.accessible_by(current_ability, :read).load
            end
          end
