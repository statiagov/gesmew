...
describe Gesmew::Admin::ProductsController, :type => :controller do
  stub_authorization!

  context "#index" do
    let(:ability_user) { stub_model(Gesmew::LegacyUser, :has_gesmew_role? => true) }

    # Regression test for #1259
    it "can find a product by SKU" do
      product = create(:product, :sku => "ABC123")
      gesmew_get :index, :q => { :sku_start => "ABC123" }
  ...
  context "adding properties to a product" do
    ...
    specify do
      gesmew_put :update, :id => product.to_param, :product => { :product_properties_attributes => { "1" => { :property_name => "Foo", :value => "bar" } } }
  ...
  context "destroying a product" do
    ...
    it "deletes all the variants (including master) for the product" do
      gesmew_delete :destroy, :id => product
  ...
  context "stock" do
    ...
    it "restricts stock location based on accessible attributes" do
      expect(Gesmew::StockLocation).to receive(:accessible_by).and_return([])
      gesmew_get :stock, :id => product
    end
  end
end
