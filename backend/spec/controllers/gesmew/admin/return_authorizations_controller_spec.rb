...
describe Gesmew::Admin::ReturnAuthorizationsController, :type => :controller do
  stub_authorization!
  ...
  describe "#load_return_authorization_reasons" do
    ...
    context "return authorization has an associated inactive reason" do
      ...
      it "loads all the active rma reasons" do
        gesmew_get :edit, id: return_authorization.to_param, order_id: return_authorization.order.to_param
    ...
    context "return authorization doesn't have an associated reason" do
      it "loads all the active rma reasons" do
        gesmew_get :new, order_id: order.to_param
        expect(assigns(:reasons)).to eq [return_authorization_reason]
      end
    end
  end
  ...
  describe "#load_return_items" do
    ...
    context '#new' do
      subject { gesmew_get :new, order_id: order.to_param }
  ...
  context '#create' do
    ...
    it "can create a return authorization" do
      subject
      expect(response).to redirect_to gesmew.admin_order_return_authorizations_path(order)
    end
  end
    ...
    context "adding an item" do
      ...
      context 'without existing items' do
        it 'creates a new item' do
          expect { subject }.to change { Gesmew::ReturnItem.count }.by(1)
        end
      end
      ...
      context 'with existing completed items' do
        ...
        it 'does not create new items' do
          expect { subject }.to_not change { Gesmew::ReturnItem.count }
