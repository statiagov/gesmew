    ...
    describe PaymentsController, :type => :controller do
      ...
      context "with a valid credit card" do
        ...
        before do
          ...
          gesmew_post :create, attributes
        end

        it "should process payment correctly" do
          expect(order.payments.count).to eq(1)
          expect(response).to redirect_to(gesmew.admin_order_payments_path(order))
          expect(order.reload.state).to eq('complete')
        end

        # Regression for #4768
        it "doesnt process the same payment twice" do
          expect(Gesmew::LogEntry.where(source: order.payments.first).count).to eq(1)
        end
      end
      ...
      context "with a backend payment method" do
        ...
        it "loads backend payment methods" do
          gesmew_get :new, :order_id => order.number
      ...
      context "order has billing address" do
        ...
        context "order does not have payments" do
          it "redirect to new payments page" do
            gesmew_get :index, { amount: 100, order_id: order.number }
            expect(response).to redirect_to(gesmew.new_admin_order_payment_path(order))
          end
        end
      ...
      context "order does not have a billing address" do
        ...
        it "should redirect to the customer details page" do
          gesmew_get :index, { amount: 100, order_id: order.number }
          expect(response).to redirect_to(gesmew.edit_admin_order_customer_path(order))
        end
      end
