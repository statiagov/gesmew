...
# the load_resource filter in Gesmew::Admin::ResourceController is prepended to the filter chain
...
describe Gesmew::Admin::ProductsController, :type => :controller do
  ...
  it "cannot find a non-existent product" do
    gesmew_get :edit, :id => "non-existent-product"
    expect(response).to redirect_to(gesmew.admin_products_path)
