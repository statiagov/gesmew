...
describe "General Settings", type: :feature, js: true do
  ...
  before(:each) do
    ...
    visit gesmew.edit_admin_general_settings_path
  end
  ...
  context "editing general settings (admin)" do
    it "should be able to update the site name" do
      fill_in "store_name", with: "Gesmew Demo Site99"
      fill_in "store_mail_from_address", with: "gesmew@example.org"
      click_button "Update"

      assert_successful_update_message(:general_settings)
      expect(find("#store_name").value).to eq("Gesmew Demo Site99")
      expect(find("#store_mail_from_address").value).to eq("gesmew@example.org")
    end
  end

  context "clearing the cache" do
    it "should clear the cache" do
      expect(page).to_not have_content(Gesmew.t(:clear_cache_ok))
      expect(page).to have_content(Gesmew.t(:clear_cache_warning))

      click_button "Clear Cache"

      expect(page).to have_content(Gesmew.t(:clear_cache_ok))
    end
  end
end
