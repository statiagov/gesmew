...
describe "New Order", :type => :feature do
  ...
  it "does check if you have a billing address before letting you add shipments" do
    ...
    expect(current_path).to eql(gesmew.edit_admin_order_customer_path(Gesmew::Order.last))
  end
  ...
  it "completes new order successfully without using the cart", js: true do
    ...
    expect(current_path).to eql(gesmew.admin_order_payments_path(Gesmew::Order.last))
    click_icon "capture"
  ...
  context "without a delivery step", js: true do
    before do
      allow(Gesmew::Order).to receive_messages checkout_step_names: [:address, :payment, :confirm, :complete]
    end
  ...
  context "customer with default credit card", js: true do
    ...
    it "transitions to delivery not to complete" do
      ...
      click_button "Update"
      expect(Gesmew::Order.last.state).to eq 'delivery'
    end
  end
