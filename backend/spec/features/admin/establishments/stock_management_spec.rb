...
describe "Stock Management", type: :feature, js: true do
  ...
  context "given a establishment with a variant and a stock location" do
    let!(:stock_location) { create(:stock_location, name: 'Default') }
    let!(:establishment) { create(:establishment, name: 'apache baseball cap', price: 10) }
    let!(:variant) { establishment.master }

    before do
      stock_location.stock_item(variant).update_column(:count_on_hand, 10)
      visit gesmew.stock_admin_product_path(establishment)
    end
    ...
    # to happen. By ensuring that we're still on an /admin/establishments URL, we
    ...
    it "can toggle backorderable for the second variant stock item", js: true do
      ...
      expect(page.current_url).to include("/admin/establishments")
    end
    ...
    context "with multiple variants" do
      before do
        variant = establishment.variants.create!(sku: 'GesmewC')
    ...
    context "with no stock location" do
      before do
        @establishment = create(:establishment, name: 'apache baseball cap', price: 10)
        v = @establishment.variants.create!(sku: 'FOOBAR')
        Gesmew::StockLocation.delete_all

        visit gesmew.stock_admin_product_path(@establishment)
      end

      it "redirects to stock locations page" do
        expect(page).to have_content(Gesmew.t(:stock_management_requires_a_stock_location))
