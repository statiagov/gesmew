module Gesmew
  module Admin
    class PaymentsController < Gesmew::Admin::BaseController
      ...
      def create
        ...
        begin
          if @payment.save
            ...
            flash[:error] = Gesmew.t(:payment_could_not_be_created)
            render :new
          end
        rescue Gesmew::Core::GatewayError => e
          invoke_callbacks(:create, :fails)
      ...
      def fire
        ...
        if @payment.send("#{event}!")
          flash[:success] = Gesmew.t(:payment_updated)
        else
          flash[:error] = Gesmew.t(:cannot_perform_operation)
        end
      rescue Gesmew::Core::GatewayError => ge
