module Gesmew
  module Admin
    class TaxonsController < Gesmew::Admin::BaseController
      ...
      def search
        if params[:ids]
          @taxons = Gesmew::Taxon.where(:id => params[:ids].split(','))
        else
          @taxons = Gesmew::Taxon.limit(20).ransack(:name_cont => params[:q]).result
        end
      end
      ...
      def create
        ...
        if @taxon.save
          ...
          flash[:error] = Gesmew.t('errors.messages.could_not_create_taxon')
          respond_with(@taxon) do |format|
