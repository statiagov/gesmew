    ...
    class PaymentMethodsController < ResourceController
      ...
      def create
        ...
        if @payment_method.save
          ...
          flash[:success] = Gesmew.t(:successfully_created, :resource => Gesmew.t(:payment_method))
      ...
      def update
        ...
        if @payment_method.update_attributes(attributes)
          ...
          flash[:success] = Gesmew.t(:successfully_updated, :resource => Gesmew.t(:payment_method))
          redirect_to edit_admin_payment_method_path(@payment_method)
        else
      ...
      def validate_payment_method_provider
        valid_payment_methods = Rails.application.config.gesmew.payment_methods.map(&:to_s)
        if !valid_payment_methods.include?(params[:payment_method][:type])
          flash[:error] = Gesmew.t(:invalid_payment_provider)
