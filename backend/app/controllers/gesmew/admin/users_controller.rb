module Gesmew
  module Admin
    class UsersController < ResourceController
      rescue_from Gesmew::Core::DestroyWithOrdersError, :with => :user_destroy_with_orders_error
      ...
      def orders
        params[:q] ||= {}
        @search = Gesmew::Order.reverse_chronological.ransack(params[:q].merge(user_id_eq: @user.id))
        @orders = @search.result.page(params[:page]).per(Gesmew::Config[:admin_products_per_page])
      end

      def items
        params[:q] ||= {}
        @search = Gesmew::Order.includes(
          line_items: {
      ...
      protected
        ...
        def collection
          ...
          if request.xhr? && params[:q].present?
            @collection = Gesmew.user_class.includes(:bill_address, :ship_address)
                              .where("gesmew_users.email #{LIKE} :search
                                     OR (gesmew_addresses.firstname #{LIKE} :search AND gesmew_addresses.id = gesmew_users.bill_address_id)
                                     OR (gesmew_addresses.lastname  #{LIKE} :search AND gesmew_addresses.id = gesmew_users.bill_address_id)
                                     OR (gesmew_addresses.firstname #{LIKE} :search AND gesmew_addresses.id = gesmew_users.ship_address_id)
                                     OR (gesmew_addresses.lastname  #{LIKE} :search AND gesmew_addresses.id = gesmew_users.ship_address_id)",
                                    { :search => "#{params[:q].strip}%" })
                              .limit(params[:limit] || 100)
          else
            @search = Gesmew.user_class.ransack(params[:q])
            @collection = @search.result.page(params[:page]).per(Gesmew::Config[:admin_products_per_page])
          end
        end
      ...
      def user_destroy_with_orders_error
        ...
        render status: :forbidden, text: Gesmew.t(:error_user_destroy_with_orders)
      end
