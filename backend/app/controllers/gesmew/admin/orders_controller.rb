module Gesmew
  module Admin
    class OrdersController < Gesmew::Admin::BaseController
      ...
      def index
        params[:q] ||= {}
        params[:q][:completed_at_not_null] ||= '1' if Gesmew::Config[:show_only_complete_orders_by_default]
        ...
        # e.g. SELECT  DISTINCT DISTINCT "gesmew_orders".id, "gesmew_orders"."created_at" AS alias_0 FROM "gesmew_orders"
        # see https://github.com/gesmew/gesmew/pull/3919
        @orders = @search.result(distinct: true).
          page(params[:page]).
          per(params[:per_page] || Gesmew::Config[:orders_per_page])
      ...
      def update
        if @order.update_attributes(params[:order]) && @order.line_items.present?
          ...
          @order.errors.add(:line_items, Gesmew.t('errors.messages.blank')) if @order.line_items.empty?
        end
      ...
      def cancel
        @order.canceled_by(try_gesmew_current_user)
        flash[:success] = Gesmew.t(:order_canceled)
        redirect_to :back
      end

      def resume
        @order.resume!
        flash[:success] = Gesmew.t(:order_resumed)
        redirect_to :back
      end

      def approve
        @order.approved_by(try_gesmew_current_user)
        flash[:success] = Gesmew.t(:order_approved)
        redirect_to :back
      end

      def resend
        OrderMailer.confirm_email(@order.id, true).deliver_later
        flash[:success] = Gesmew.t(:order_email_resent)
