module Gesmew
  module Admin
    class CustomerReturnsController < ResourceController
      belongs_to 'gesmew/order', find_by: :number
      ...
      def build_resource
        Gesmew::CustomerReturn.new
      end

      def find_resource
        Gesmew::CustomerReturn.accessible_by(current_ability, :read).find(params[:id])
      end

      def collection
        parent # trigger loading the order
        @collection ||= Gesmew::ReturnItem
      ...
      def build_return_items_from_params
        ...
        @customer_return.return_items = return_items_params.map do |item_params|
          ...
          return_item = item_params[:id] ? Gesmew::ReturnItem.find(item_params[:id]) : Gesmew::ReturnItem.new
