...
handle_delete = (e, data) ->
  ...
  response = confirm(Gesmew.translations.are_you_sure_delete)
  if response
    $.ajax
      type: "POST",
      dataType: "json",
...
root.setup_taxonomy_tree = (taxonomy_id) ->
  ...
  if taxonomy_id != undefined
    ...
    root.base_url = Gesmew.url(Gesmew.routes.taxonomy_taxons_path)

    $.ajax
      url: Gesmew.url(base_url.path().replace("/taxons", "/jstree")).toString(),
      data:
        token: Gesmew.api_key
      success: (taxonomy) ->
        last_rollback = null
        ...
        conf =
          json_data:
            ...
            ajax:
              url: (e) ->
                Gesmew.url(base_url.path() + '/' + e.prop('id') + '/jstree' + '?token=' + Gesmew.api_key).toString()
          themes:
            theme: "gesmew",
            url: Gesmew.url(Gesmew.routes.jstree_theme_path)
          strings:
            new_node: new_taxon,
            loading: Gesmew.translations.loading + "..."
          crrm:
            move:
              check_move: (m) ->
                position = m.cp
