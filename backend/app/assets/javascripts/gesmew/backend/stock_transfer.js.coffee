$ ->
  ...
  class TransferLocations
    constructor: ->
      ...
      $.getJSON Gesmew.url(Gesmew.routes.stock_locations_api) + '?token=' + Gesmew.api_key, (data) =>
  ...
  class TransferVariants
    ...
    _search_transfer_variants: ->
      @build_select(Gesmew.url(Gesmew.routes.variants_api), 'product_name_or_sku_cont')

    _search_transfer_stock_items: ->
      stock_location_id = $('#transfer_source_location_id').val()
      @build_select(Gesmew.url(Gesmew.routes.stock_locations_api + "/#{stock_location_id}/stock_items"),
