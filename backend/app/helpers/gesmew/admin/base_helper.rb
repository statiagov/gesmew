    ...
    module BaseHelper
      def flash_alert flash
        if flash.present?
          ...
          flash_div = content_tag(:div, message, class: "alert alert-#{flash_class} alert-dismissible")
      ...
      def datepicker_field_value(date)
        unless date.blank?
          l(date, format: Gesmew.t('date_picker.format', default: '%Y/%m/%d'))
        else
          nil
        end
      end
      ...
      def preference_fields(object, form)
        ...
        if object.has_preference?(key)
          form.label("preferred_#{key}", Gesmew.t(key) + ": ") +
      ...
      def link_to_icon_remove_fields(f)
        ...
        link_to_with_icon('delete', '', url, class: "gesmew_remove_fields btn btn-sm btn-danger", data: {action: 'remove'}, title: Gesmew.t(:remove)) + f.hidden_field(:_destroy)
      end

      def gesmew_dom_id(record)
        dom_id(record, 'gesmew')
      end
