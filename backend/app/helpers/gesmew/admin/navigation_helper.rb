    ...
    module NavigationHelper
      ...
      def tab(*args)
        ...
        selected = if options[:match_path].is_a? Regexp
          ...
          request.fullpath.starts_with?("#{gesmew.admin_path}#{options[:match_path]}")
        else
      ...
      def main_menu_tree text, icon: nil, sub_menu: nil, url: '#'
        content_tag :li, class: 'sidebar-menu-item' do
          ...
          render(partial: "gesmew/admin/shared/sub_menu/#{sub_menu}")
        end
      end
      ...
      def klass_for(name)
        ...
        ["Gesmew::#{model_name.classify}", model_name.classify, model_name.gsub('_', '/').classify].find do |t|
          t.safe_constantize
      ...
      def link_to_clone(resource, options={})
        ...
        link_to_with_icon('clone', Gesmew.t(:clone), clone_object_url(resource), options)
      end

      def link_to_new(resource)
        options[:data] = { action: 'new' }
        options[:class] = "btn btn-success btn-sm"
        link_to_with_icon('plus', Gesmew.t(:new), edit_object_url(resource))
      end
      ...
      def link_to_edit(resource, options={})
        ...
        options[:class] = "btn btn-primary btn-sm"
        link_to_with_icon('edit', Gesmew.t(:edit), url, options)
      end
      ...
      def link_to_delete(resource, options={})
        ...
        name = options[:name] || Gesmew.t(:delete)
        options[:class] = "btn btn-danger btn-sm delete-resource"
        options[:data] = { confirm: Gesmew.t(:are_you_sure), action: 'remove' }
