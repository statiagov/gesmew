...
Gesmew currently supports RESTful access to the resources listed in the sidebar
on the right &raquo;
...
Developers communicate with the Gesmew API using the [JSON](http://www.json.org) data format. Requests for data are communicated in the standard manner using the HTTP protocol.

### Making an API Call

You will need an authentication token to access the API. These keys can be generated on the user edit screen within the admin interface. To make a request to the API, pass a `X-Gesmew-Token` header along with the request:

```bash
$ curl --header "X-Gesmew-Token: YOUR_KEY_HERE" http://example.com/api/establishments.json```
...
```bash
$ curl http://example.com/api/establishments.json?token=YOUR_KEY_HERE```
...
The following are some simple rules that all Gesmew API endpoints comply with.
...
3. Both create and update requests will return Gesmew\'s representation of the data upon success.
...
6. Requests that list collections, such as /api/establishments will return a limited result set back.
...
`app/views/gesmew/api/establishments/show.v1.rabl`, it will take precedence over the
...
at `app/views/gesmew/api/establishments`. The template must be given a unique name that
...
`app/views/gesmew/api/establishments/small_show.v1.rabl`. The `v1` part of the filename
...
`http://example.com/api/establishments/1?template=small_show`. The API component of
Gesmew will then detect this parameter, find the template, and then use this to
render the response.

***
Due to [the way this implemented](https://github.com/gesmew/gesmew/blob/v2.3.1/api/lib/gesmew/api/responders/rabl_template.rb#L5-L18)
