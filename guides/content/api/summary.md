...
```bash
$ curl --header "X-Gesmew-Token: YOUR_KEY_HERE" http://example.com/api/products.json```
...
`app/views/gesmew/api/products/show.v1.rabl`, it will take precedence over the
...
at `app/views/gesmew/api/products`. The template must be given a unique name that
...
`app/views/gesmew/api/products/small_show.v1.rabl`. The `v1` part of the filename
