...
```ruby
p = Gesmew::Product.create :name => 'some product', :price => 10.0,
...
column, this means:
```ruby
Gesmew::ProductProperty.create :property => size_prop, :product => p, :value => size_info
```
...
```ruby
v = Gesmew::Variant.create :product => p, :sku => "some_sku_code", :price => NNNN
```
...
v = Gesmew::Variant.create :product => p, :sku => sku, :price => price
...
increasing size or by alphabetical order. The *Gesmew::OptionValue* model uses
...
img = Gesmew::Image.create(:attachment => File.open(path), :viewable => product.master)
