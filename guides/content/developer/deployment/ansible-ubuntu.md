...
Along with the [Manual Ubuntu Deployment Guide](/developer/manual-ubuntu.html), Gesmew can also be set up using [Ansible](http://ansibleworks.com). From Ansible's website:
...
webserver_name: gesmew.example.com
deploy_directory: /data/gesmew
app_name: gesmew
...
You can also choose to seed the Gesmew store with some sample data by running this command:

    bundle exec cap production gesmew_sample:load
