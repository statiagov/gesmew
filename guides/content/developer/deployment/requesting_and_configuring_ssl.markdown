...
this step. In inspection to get an SSL certificate from a Certificate
...
Example: "GesmewCommerce, Inc." If you do not have a legal registered
...
Gesmew Hosting servers have this command available so it's best to run
...
$ openssl req -new -newkey rsa:2048 -nodes -out www_example_com.csr
  ...
  Chase/O=GesmewCommerce, Inc /OU= /CN=www.example.com"
```
...
Copy the CRT file onto the server and save it to `/etc/ssl/gesmew.crt` directory.

Move the private key (KEY file) to the `/etc/ssl/gesmew.key` directory.
