...
You may see what price a product would be in the current currency (`Gesmew::Config[:currency]`) by calling the `price` method on that instance:
...
If you have products where prices are greater than 9999999.99 then you should decorate `Gesmew::Price` and customize `maximum_amount` method in order to support the prices.
