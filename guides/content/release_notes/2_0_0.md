...
Complex Gesmew stores require sophisticated shipping and warehouse logic that Gesmew hasn't had a general solution for until now. Split shipments in Gesmew allows for multiple shipments per order and for those shipments to be shipped from multiple locations.
...
The Gesmew API now has support for "checking out" an order. This API
...
* Allow `meta_description` on `Gesmew::Product` to be as long as `text` will allow, rather than `string`. <%= issue 2611 %>
