...
The adjustments system in Gesmew has undergone a large portion of work. Adjustments (typically originating from promotions and taxes) can now be applied at a line item, shipment or order level.
...
Also worth noting is that the number of callbacks triggered when any aspect of an order is updated has been greatly reduced, which should lead up to speed-ups in stores. An example of this would be in prior versions of Gesmew, an order would trigger an update on all its adjustments when it updated. With the new system, only line items or shipments that change will have their adjustments updated. 
...
To fix this bug, we have moved the location of the assets to `vendor/assets`. Frontend's assets are now placed in `vendor/assets/gesmew/frontend` and Backend's are in `vendor/assets/gesmew/backend`. 
...
*   Switched to using friendly_id for permalink generation. This meant that we needed to rename `Gesmew::Product`'s `permalink` field to `slug`.
...
*   A *channel* column was added to the gesmew_orders table. Users can set
...
* Order token can now be passed as a header: `X-Gesmew-Order-Token`. #4148
...
* Implemented a basic Risk Assessment feature in Gesmew Backend. Viewing any Order's edit page now shows the following, with a status indicator:
