...
Product Groups component has been extracted to a standalone gesmew
...
The API component of Gesmew has undergone a major rewrite in order to
...
## Removed images association of `Gesmew::Product`

`Gesmew::Product` association with `viewable` has been moved `master`
variant of
the product. The change is backward compatible and require no upgrade
modification.

A call to the `images` method on a `Gesmew::Product` will return all the
