...
describe "Visiting Products", type: :feature, inaccessible: true do
  ...
  describe 'meta tags and title' do
    let(:jersey) { Gesmew::Product.find_by_name('Ruby on Rails Baseball Jersey') }
  ...
  context "using Russian Rubles as a currency" do
    ...
    let!(:product) do
      product = Gesmew::Product.find_by_name("Ruby on Rails Ringer T-Shirt")
      product.price = 19.99
      product.tap(&:save)
    end
  ...
  context "a product with variants" do
    let(:product) { Gesmew::Product.find_by_name("Ruby on Rails Baseball Jersey") }
  ...
  it "should return the correct title when displaying a single product" do
    product = Gesmew::Product.find_by_name("Ruby on Rails Baseball Jersey")
    click_link product.name
