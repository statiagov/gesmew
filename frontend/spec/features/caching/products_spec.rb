...
describe 'products', :type => :feature, :caching => true do
  ...
  before do
    ...
    assert_written_to_cache("views/en/USD/gesmew/products/all--#{product.updated_at.utc.to_s(:number)}")
    assert_written_to_cache("views/en/USD/gesmew/products/#{product.id}-#{product.updated_at.utc.to_s(:number)}")
  ...
  it "busts the cache when all products are deleted" do
    ...
    product2.destroy
    visit gesmew.root_path
    assert_written_to_cache("views/en/USD/gesmew/products/all--#{Date.today.to_s(:number)}-0")
  ...
  it "busts the cache when the newest product is deleted" do
    product.destroy
    visit gesmew.root_path
    assert_written_to_cache("views/en/USD/gesmew/products/all--#{product2.updated_at.utc.to_s(:number)}")
    expect(cache_writes.count).to eq(1)
  end
