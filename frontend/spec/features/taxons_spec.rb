...
describe "viewing establishments", type: :feature, inaccessible: true do
  ...
  let!(:establishment) do
    establishment = create(:establishment, name: "Superman T-Shirt")
    establishment.taxons << t_shirts
  end
  ...
  it "can see a taxon's establishments, even if that taxon has child taxons" do
  ...
  context "taxon pages" do
    ...
    it "should be able to visit brand Ruby on Rails" do
      ...
      expect(page.all('#establishments .establishment-list-item').size).to eq(7)
    ...
    it "should be able to visit category Clothing" do
      ...
      expect(page.all('#establishments .establishment-list-item').size).to eq(5)
      tmp = page.all('#establishments .establishment-list-item a').map(&:text).flatten.compact
      tmp.delete("")
      expect(tmp.sort!).to eq(["Apache Baseball Jersey",
