module Gesmew
  ...
  # is waranted.
  class CheckoutController < Gesmew::StoreController
    ...
    helper 'gesmew/orders'

    rescue_from Gesmew::Core::GatewayError, with: :rescue_from_gesmew_gateway_error
    ...
    def update
      if @order.update_from_params(params, permitted_checkout_attributes, request.headers.env)
        ...
        if @order.completed?
          @current_order = nil
          flash.notice = Gesmew.t(:order_processed_successfully)
    ...
    def load_order_with_lock
      ...
      redirect_to(gesmew.cart_path) && return unless @order
    end
      ...
      if params[:order] && params[:order][:state_lock_version]
        @order.with_lock do
          unless @order.state_lock_version == params[:order].delete(:state_lock_version).to_i
            flash[:error] = Gesmew.t(:order_already_updated)
            redirect_to(checkout_state_path(@order.state)) && return
          end
    ...
    def ensure_order_not_completed
      redirect_to gesmew.cart_path if @order.completed?
    end

    def ensure_sufficient_stock_lines
      if @order.insufficient_stock_lines.present?
        flash[:error] = Gesmew.t(:inventory_error_flash_for_insufficient_quantity)
        redirect_to gesmew.cart_path
      end
    end
    ...
    def completion_route(custom_params = nil)
      gesmew.order_path(@order, custom_params)
    end
    ...
    def before_delivery
      ...
      @differentiator = Gesmew::Stock::Differentiator.new(@order, packages)
    end
    ...
    def before_payment
      ...
      if try_gesmew_current_user && try_gesmew_current_user.respond_to?(:payment_sources)
        @payment_sources = try_gesmew_current_user.payment_sources
      end
    end

    def rescue_from_gesmew_gateway_error(exception)
      flash.now[:error] = Gesmew.t(:gesmew_gateway_error_flash_for_checkout)
