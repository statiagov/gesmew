Gesmew.ready ($) ->
  Gesmew.onAddress = () ->
    if ($ '#checkout_form_address').is('*')
      ...
      Gesmew.updateState = (region) ->
        countryId = getCountryId(region)
        if countryId?
          unless Gesmew.Checkout[countryId]?
            $.get Gesmew.routes.states_search, {country_id: countryId}, (data) ->
              Gesmew.Checkout[countryId] =
                states: data.states
                states_required: data.states_required
              Gesmew.fillStates(Gesmew.Checkout[countryId], region)
          else
            Gesmew.fillStates(Gesmew.Checkout[countryId], region)

      Gesmew.fillStates = (data, region) ->
      ...
      ($ '#bcountry select').change ->
        Gesmew.updateState 'b'

      ($ '#scountry select').change ->
        Gesmew.updateState 's'
