# Use singleton class Gesmew::Preferences::Store.instance to access
#
...
module Gesmew::Preferences
  ...
  class StoreInstance
    ...
    def exist?(key)
      @cache.exist?(key) ||
      should_persist? && Gesmew::Preference.where(:key => key).exists?
    end
    ...
    def get(key)
      ...
      if should_persist?
        ...
        if preference = Gesmew::Preference.find_by_key(key)
          # it does exist
    ...
    def persist(cache_key, value)
      ...
      preference = Gesmew::Preference.where(:key => cache_key).first_or_initialize
    ...
    def destroy(cache_key)
      ...
      preference = Gesmew::Preference.find_by_key(cache_key)
      preference.destroy if preference
    end

    def should_persist?
      @persistence and Gesmew::Preference.table_exists?
    end
