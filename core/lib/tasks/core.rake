...
use rake db:load_file[/absolute/path/to/sample/filename.rb]}
  ...
  task :load_dir , [:dir] => :environment do |t, args|
    ...
    ruby_files.sort.each do |fixture , ruby_file|
      ...
      if File.exists?(File.join(Rails.root, "db/default/gesmew", "#{fixture}.rb"))
        ruby_file = File.expand_path(File.join(Rails.root, "db/default/gesmew", "#{fixture}.rb"))
      end
  ...
  task :bootstrap  do
    ...
    load_defaults  = Gesmew::Country.count == 0
    ...
    if load_sample
      ...
      Rake::Task["gesmew_sample:load"].invoke
    end
