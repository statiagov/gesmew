...
# within the code that Gesmew's test suite covers.
#
...
module Gesmew
    ...
    used_translations.map { |a| a.split('.') }.each do |translation_keys|
      ...
      translation_keys.each do |key|
        begin
          ...
          unless Gesmew.missing_translation_messages.include?(error)
            Gesmew.missing_translation_messages << error
          end
        end
      end
    end
  ...
  def self.check_unused_translations
    ...
    translation_diff.each do |translation|
      Gesmew.unused_translation_messages << "#{translation} (#{I18n.locale})"
    end
  end
  ...
  def self.translations
    @translations ||= I18n.backend.send(:translations)[I18n.locale][:gesmew]
  end
end
...
RSpec.configure do |config|
  ...
  if ENV['CHECK_TRANSLATIONS']
    config.after :suite do
      ...
      if Gesmew.missing_translation_messages.any?
        puts "\nThere are missing translations within Gesmew:"
        puts Gesmew.missing_translation_messages.sort
        exit(1)
      end

      Gesmew.check_unused_translations
      if false && Gesmew.unused_translation_messages.any?
        puts "\nThere are unused translations within Gesmew:"
        puts Gesmew.unused_translation_messages.sort
        exit(1)
      end
    end
  end
