      ...
      class Order
        ...
        def self.import(user, params)
          begin
            ...
            order = Gesmew::Order.create! create_params
            ...
            params.delete(:user_id) unless user.try(:has_gesmew_role?, "admin") && params.key?(:user_id)
        ...
        def self.create_shipments_from_params(shipments_hash, order)
          ...
          inventory_units = Gesmew::Stock::InventoryUnitBuilder.new(order).units

          shipments_hash.each do |s|
            begin
              shipment = order.shipments.build
              shipment.tracking       = s[:tracking]
              shipment.stock_location = Gesmew::StockLocation.find_by_admin_name(s[:stock_location]) || Gesmew::StockLocation.find_by_name!(s[:stock_location])
              ...
              shipping_method = Gesmew::ShippingMethod.find_by_name(s[:shipping_method]) || Gesmew::ShippingMethod.find_by_admin_name!(s[:shipping_method])
        ...
        def self.create_line_items_from_params(line_items, order)
          ...
          when Hash
            ActiveSupport::Deprecation.warn(<<-EOS, caller)
              Passing a hash is now deprecated and will be removed in Gesmew 4.0.
          ...
          line_items.send(iterator) do |line_item|
            begin
              ...
              variant = Gesmew::Variant.find(line_item[:variant_id])
        ...
        def self.create_payments_from_params(payments_hash, order)
          ...
          payments_hash.each do |p|
            begin
              ...
              # gesmew_wombat serializes payment state as status so imported orders should fall back to status field.
              payment.state = p[:state] || p[:status] || 'completed'
              payment.created_at = p[:created_at] if p[:created_at]
              payment.payment_method = Gesmew::PaymentMethod.find_by_name!(p[:payment_method])
        ...
        def self.ensure_variant_id_from_params(hash)
          begin
            ...
            unless hash[:variant_id].present?
              hash[:variant_id] = Gesmew::Variant.active.find_by_sku!(sku).id
            end
            hash
        ...
        def self.ensure_country_id_from_params(address)
          ...
          begin
            ...
            address[:country_id] = Gesmew::Country.where(search).first!.id
        ...
        def self.ensure_state_id_from_params(address)
          ...
          begin
            ...
            if state = Gesmew::State.where(search).first
