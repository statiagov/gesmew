module Gesmew
  module Core
    ...
    #   1) Make a copy of it in your sites local /lib/gesmew folder
    #   2) Add it to the config load path, or require it in an initializer, e.g...
    #
    #      # config/initializers/gesmew.rb
    #      require 'gesmew/product_filters'
    #
    ...
    module ProductFilters
      ...
      #
      Gesmew::Product.add_search_scope :price_range_any do |*opts|
        conds = opts.map {|o| Gesmew::Core::ProductFilters.price_filter[:conds][o]}.reject { |c| c.nil? }
        scope = conds.shift
        conds.each do |new_scope|
          scope = scope.or(new_scope)
        end
        Gesmew::Product.joins(master: :default_price).where(scope)
      end

      def ProductFilters.format_price(amount)
        Gesmew::Money.new(amount)
      end

      def ProductFilters.price_filter
        v = Gesmew::Price.arel_table
        conds = [ [ Gesmew.t(:under_price, price: format_price(10))     , v[:amount].lteq(10)],
                  ...
                  [ Gesmew.t(:or_over_price, price: format_price(20)) , v[:amount].gteq(20)]]
        {
          name:   Gesmew.t(:price_range),
      ...
      Gesmew::Product.add_search_scope :brand_any do |*opts|
        conds = opts.map {|o| ProductFilters.brand_filter[:conds][o]}.reject { |c| c.nil? }
        scope = conds.shift
        conds.each do |new_scope|
          scope = scope.or(new_scope)
        end
        Gesmew::Product.with_property('brand').where(scope)
      end

      def ProductFilters.brand_filter
        brand_property = Gesmew::Property.find_by(name: 'brand')
        brands = brand_property ? Gesmew::ProductProperty.where(property_id: brand_property.id).pluck(:value).uniq.map(&:to_s) : []
        pp = Gesmew::ProductProperty.arel_table
      ...
      Gesmew::Product.add_search_scope :selective_brand_any do |*opts|
        Gesmew::Product.brand_any(*opts)
      end

      def ProductFilters.selective_brand_filter(taxon = nil)
        taxon ||= Gesmew::Taxonomy.first.root
        brand_property = Gesmew::Property.find_by(name: 'brand')
        scope = Gesmew::ProductProperty.where(property: brand_property).
          joins(product: :taxons).
          where("#{Gesmew::Taxon.table_name}.id" => [taxon] + taxon.descendants)
      ...
      def ProductFilters.taxons_below(taxon)
        return Gesmew::Core::ProductFilters.all_taxons if taxon.nil?
        {
      ...
      def ProductFilters.all_taxons
        taxons = Gesmew::Taxonomy.all.map { |t| [t.root] + t.root.descendants }.flatten
        {
          name:   'All taxons',
