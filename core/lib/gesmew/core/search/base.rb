    ...
    module Search
      class Base
        ...
        def initialize(params)
          self.current_currency = Gesmew::Config[:currency]
          @properties = {}
          prepare(params)
        end
        ...
        protected
          ...
          def prepare(params)
            @properties[:taxon] = params[:taxon].blank? ? nil : Gesmew::Taxon.find(params[:taxon])
            @properties[:keywords] = params[:keywords]
            @properties[:search] = params[:search]
            @properties[:include_images] = params[:include_images]

            per_page = params[:per_page].to_i
            @properties[:per_page] = per_page > 0 ? per_page : Gesmew::Config[:products_per_page]
