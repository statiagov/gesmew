...
describe Gesmew::Utf8Cleaner do
  it "should strip out invalid utf-8" do
    ...
    test_strings.each do |input, output|
      ...
      expect(Gesmew::Utf8Cleaner.strip_invalid_utf8(input)).to eq(output)
    end
  end
...
YAML
      ...
      expect(answer.valid_encoding?).to be false
      Gesmew::Utf8Cleaner.recursively_strip_invalid_utf8!(data, true)
      expect(answer).to eq("two")
      expect(answer.encoding).to eq(Encoding::UTF_8)
      expect(answer.valid_encoding?).to be true
