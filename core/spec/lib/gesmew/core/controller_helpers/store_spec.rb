...
describe Gesmew::Core::ControllerHelpers::Store, type: :controller do
  ...
  describe "#current_tax_zone" do
    ...
    context "when there is a default tax zone" do
      let(:default_zone) { Gesmew::Zone.new }

      before do
        allow(Gesmew::Zone).to receive(:default_tax).and_return(default_zone)
      end
      ...
      context "when there is a current order" do
        let(:other_zone) { Gesmew::Zone.new }
    ...
    context "when there is no default tax zone" do
      before do
        allow(Gesmew::Zone).to receive(:default_tax).and_return(nil)
      end
