...
class FakesController < ApplicationController
  include Gesmew::Core::ControllerHelpers::Order
end

describe Gesmew::Core::ControllerHelpers::Order, type: :controller do
  ...
  describe '#simple_current_order' do
    ...
    it 'returns Gesmew::Order instance' do
  ...
  describe '#current_order' do
    ...
    context 'create_order_if_necessary option is true' do
      it 'creates new order' do
        ...
        }.to change(Gesmew::Order, :count).to(1)
      end

      it 'assigns the current_store id' do
        controller.current_order(create_order_if_necessary: true)
        expect(Gesmew::Order.last.store_id).to eq store.id
      end
    end
  end
  ...
  describe '#associate_user' do
    before do
      allow(controller).to receive_messages(current_order: order, try_gesmew_current_user: user)
    end
    context "user's email is blank" do
      let(:user) { create(:user, email: '') }
      it 'calls Gesmew::Order#associate_user! method' do
        expect_any_instance_of(Gesmew::Order).to receive(:associate_user!)
        controller.associate_user
      end
    end
    context "user isn't blank" do
      it 'does not calls Gesmew::Order#associate_user! method' do
        expect_any_instance_of(Gesmew::Order).not_to receive(:associate_user!)
        controller.associate_user
      end
    end
  end
  ...
  describe '#set_current_order' do
    ...
    context 'when current order not equal to users incomplete orders' do
      ...
      it 'calls Gesmew::Order#merge! method' do
