...
describe "exchanges:charge_unreturned_items" do
  ...
  context "there are no unreturned items" do
    it { expect { subject.invoke }.not_to change { Gesmew::Order.count } }
  end
  ...
  context "there are unreturned items" do
    ...
    context "fewer than the config allowed days have passed" do
      ...
      it "does not create a new order" do
        expect { subject.invoke }.not_to change { Gesmew::Order.count }
      end
    end
    ...
    context "more than the config allowed days have passed" do
      ...
      it "creates a new completed order" do
        expect { subject.invoke }.to change { Gesmew::Order.count }
        expect(Gesmew::Order.last).to be_completed
      end

      it "moves the shipment for the unreturned items to the new order" do
        subject.invoke
        new_order = Gesmew::Order.last
        expect(new_order.shipments.count).to eq 1
        expect(return_item_2.reload.exchange_shipment.order).to eq Gesmew::Order.last
      end

      it "creates line items on the order for the unreturned items" do
        subject.invoke
        expect(Gesmew::Order.last.line_items.map(&:variant)).to eq [return_item_2.exchange_variant]
      end

      it "associates the exchanges inventory units with the new line items" do
        subject.invoke
        expect(return_item_2.reload.exchange_inventory_unit.try(:line_item).try(:order)).to eq Gesmew::Order.last
      end
      ...
      it "associates the store of the original order with the exchange order" do
        allow_any_instance_of(Gesmew::Order).to receive(:store_id).and_return(123)

        expect(Gesmew::Order).to receive(:create!).once.with(hash_including({store_id: 123})) { |attrs| Gesmew::Order.new(attrs.except(:store_id)).tap(&:save!) }
        subject.invoke
      end
      ...
      context "there is no card from the previous order" do
        ...
        before { allow_any_instance_of(Gesmew::Order).to receive(:valid_credit_cards) { [] } }
