...
describe Gesmew::Preferences::Store, :type => :model do
  before :each do
    @store = Gesmew::Preferences::StoreInstance.new
  end
  ...
  it "will return db value when cache is emtpy and cache the db value" do
    preference = Gesmew::Preference.where(:key => 'test').first_or_initialize
